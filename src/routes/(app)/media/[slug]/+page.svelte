...
<article class="mx-auto my-8 max-w-[900px] px-4">
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-[200px_1fr]">
        {#if item.coverImageUrl}
            <img
                class="w-full rounded-lg sm:max-w-[200px]"
        ...
        <div>
            ...
            <ul
                class="m-0 mb-3 flex list-none flex-wrap gap-4 p-0 text-sm text-gray-500"
            >
                <li class="rounded bg-gray-200 px-2 py-0.5 capitalize">
                    {item.mediaType}
                </li>
                ...
                {#if metadata && isMetadataType(metadata, "tv")}
                    {#if metadata.number_of_seasons}
                        <li>
                            {metadata.number_of_seasons} season{metadata.number_of_seasons ===
                            1
                                ? ""
                                : "s"}
                        </li>
                    ...
                    {#if metadata.number_of_episodes}
                        <li>
                            {metadata.number_of_episodes} episode{metadata.number_of_episodes ===
                            1
                                ? ""
                                : "s"}
                        </li>
                ...
                {#if metadata && isMetadataType(metadata, "anime")}
                    {#if metadata.episodes}
                        <li>
                            {metadata.episodes} episode{metadata.episodes === 1
                                ? ""
                                : "s"}
                        </li>
                    {/if}
                ...
                {#if metadata && isMetadataType(metadata, "manga")}
                    {#if metadata.chapters}
                        <li>
                            {metadata.chapters} chapter{metadata.chapters === 1
                                ? ""
                                : "s"}
                        </li>
                    {/if}
                    {#if metadata.volumes}
                        <li>
                            {metadata.volumes} volume{metadata.volumes === 1
                                ? ""
                                : "s"}
                        </li>
                    {/if}
            ...
            {#if genres.length}
                <ul class="m-0 mb-4 flex list-none flex-wrap gap-2 p-0">
                    {#each genres as g (g.slug)}
                        <li
                            class="rounded-full bg-gray-100 px-2.5 py-1 text-[0.8125rem] text-gray-700"
                        >
                            {g.name}
                        </li>
                    {/each}
            ...
            <div class="flex gap-2">
                ...
                {#if item.mediaType === "tv"}
                    <a
                        ...
                        class="inline-block rounded-lg border border-gray-300 px-4 py-2 text-gray-700 no-underline hover:bg-gray-50"
                    >
                        Browse episodes
                    </a>
                    ...
                    <div class="mt-2 flex flex-wrap gap-2">
                        {#each Array(metadata.number_of_seasons) as _, i}
                            <a
                                ...
                                class="rounded bg-gray-100 px-3 py-1 text-sm no-underline hover:bg-gray-200"
                            >
                                Season {i + 1}
                            </a>
                        {/each}
