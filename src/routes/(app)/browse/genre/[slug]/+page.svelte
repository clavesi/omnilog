...
function filterHref(type: string | null): string {
	const base = `/browse/genre/${data.genre.slug}`;
	if (!type) return base;
...
function pageHref(p: number): string {
	...
	return `/browse/genre/${data.genre.slug}${qs ? `?${qs}` : ""}`;
}
</script>
...
<div>
	<header class="mb-8">
		<p class="m-0 mb-1 text-sm text-text-muted">
			<a href="/browse" class="link-soft">Browse</a> / Genre
		</p>
		<h1 class="m-0 font-display text-3xl tracking-tight sm:text-4xl">{data.genre.name}</h1>
